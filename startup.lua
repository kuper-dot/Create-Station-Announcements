    ...
    if not stn.isTrainPresent() then
        ...
        if hasTrain then
            ...
            if not lastPlayed or (currentTime - lastPlayed) >= COOLDOWN_TIME then
                ...
                local operator = "<player/unmanned>"
                local destination = "<player-driven>"

                if schedule then
                    operator = getTrainOperator(schedule)

                    local result = findDestination(schedule,
                                                  station.getStationName())
                    destination = result
                end
...
function main()
    ...
    while true do
        ...
        for _, st in ipairs(peripherals) do
            ...
            --print(("[%s] Announcing %s"):format(stationName, trainName))
    end
    sleep(0.2)
        ...
        --             "disk/sm/misc/jingle.wav",
        --             "disk/sm/misc/platform.wav",
        --             string.format("disk/sm/numbers/%d.wav", platform),
        --             "disk/sm/misc/forthe.wav",
        --             string.format("disk/sm/misc/%s.wav", operator),
        --             "disk/sm/misc/serviceto.wav",
        --             string.format("disk/sm/stations/%s", stripPlatform(destName)),
        --         }
